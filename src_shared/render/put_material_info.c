...
/*   Updated: 2023/11/14 19:07:54 by rsoo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_color_instructions(t_minirt *minirt)
{
	...
	MAT_START_Y + 20, minirt->font_color, "q = R value");
	mlx_string_put(minirt->mlx, minirt->win, OBJ_START_X + 20, \
	MAT_START_Y + 40, minirt->font_color, "w = G value");
	mlx_string_put(minirt->mlx, minirt->win, OBJ_START_X + 20, \
	MAT_START_Y + 60, minirt->font_color, "e = B value");
	mlx_string_put(minirt->mlx, minirt->win, OBJ_START_X, \
	MAT_START_Y + 80, minirt->font_color, \
	"hold (q / w / e) + (up / down):");
}
...
void	put_handle_material_str(t_minirt *minirt)
{
	if (minirt->selection.selected_material_field == MATERIAL_COLOR)
	{
		...
		mlx_string_put(minirt->mlx, minirt->win, OBJ_START_X, \
		MAT_START_Y + 120, minirt->font_color, minirt->msg);
		free(minirt->msg);
	}
