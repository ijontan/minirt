...
/*   Updated: 2023/11/03 01:56:36 by itan             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 	abc.x = vec3_dot(ray->direction, cone->rot_axis)
// 		* vec3_dot(ray->direction, cone->rot_axis)
// 		- vec3_dot(ray->direction, ray->direction) * cone->cos_squared;
// 	abc.y = 2.0f * (vec3_dot(ray->direction, cone->rot_axis)
// 			* vec3_dot(co, cone->rot_axis) - vec3_dot(ray->direction, co)
// 			* cone->cos_squared);
...
t_vec3	cone_intersect(t_cone *cone, t_ray *ray)
{
	...
	co = vec3_subtract(ray->origin, cone->tip);
	sols.x = vec3_dot(ray->direction, cone->rot_axis); // d.v
	sols.y = vec3_dot(co, cone->rot_axis);             // co.v
	...
	sqrtd = vec3_dot(vec3_subtract(abc, cone->tip), cone->rot_axis);
	if (sqrtd < 0)
	{
		discriminant = vec3_dot(ray->direction, cone->rot_axis)
	...
	discriminant = vec3_dot(vec3_multiply(ray->direction, -1), cone->rot_axis)
		/ vec3_length(ray->direction);
