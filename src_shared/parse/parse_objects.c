...
/*   Updated: 2023/08/29 21:41:09 by rsoo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	p->sphere->center = parse_coordinates(p->info[1]);

	p->sphere->radius = ft_atof(p->info[2]) / 2;
...
{
	p->plane->point_on_plane = parse_coordinates(p->info[1]);
...
{
	p->cylinder->center = parse_coordinates(p->info[1]);
	...
	p->cylinder->radius = ft_atof(p->info[3]) / 2;
	if (p->cylinder->radius <= 0.0)
		exit_parse(p->info, "Cylinder", 'd');

	p->cylinder->height = ft_atof(p->info[4]);
