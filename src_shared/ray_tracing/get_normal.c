...
/*   Updated: 2023/11/15 21:26:40 by itan             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_sphere_normal(t_hit_info *hi)
{
	...
	if (hi->material->specular_map)
		hi->material->specular = get_pixel_color(hi->material->specular_map,
				uv_sphere(hi->normal, hi->material->norm_map->size));
	if (hi->material->norm_map)
	{
		n_from_map = get_pixel_vec3(hi->material->norm_map,
				uv_sphere(hi->normal, hi->material->norm_map->size));
		hi->normal = (t_vec3){.x = hi->normal.x * n_from_map.x,
			.y = hi->normal.y * n_from_map.y, .z = hi->normal.z * n_from_map.z};
		hi->normal = vec3_normalize(hi->normal);
	}
}
